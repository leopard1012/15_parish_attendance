document.addEventListener('DOMContentLoaded', () => {
    ...
    const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbymrqFk_Rmxg3x5P5O2lgV4itonMw8B429qtgGnmWFVLUm-cdsS0_bB5AId_PTGjZOb/exec';
    ...
    // 특정 연/월의 출석 데이터 가져오기
    async function fetchAttendanceData(year, month, members) {
        console.log(`${year}년 ${month}월 출석 데이터 로딩 중 (Apps Script)...`);
        ...
        return await fetchDataFromAppsScript('getAttendanceData', { year, month, members });
    }
    ...
    async function showAttendance() {
        ...
        if (!region || !group || !mokja || !year || !month) { selectionError.textContent = "모든 항목을 선택해주세요."; return; }

        // 화면 전환 및 로딩 표시
        selectionPage.style.display = 'none'; attendancePage.style.display = 'block';
        attendanceHeader.textContent = `${year}년 ${month}월 - ${region} ${group} ${mokjangDisplayName} 출석 현황`;
        ...
        try {
            ...
            const sundays = calculateSundays(parseInt(year), parseInt(month)); // 'M/D' 형식
            if (sundays.length === 0) { showInfoMessage('선택된 월에 일요일이 없습니다.', tableContainer); return; }

            currentAttendanceData = await fetchAttendanceData(year, month, currentMembers); // Apps Script 호출

            // 테이블 생성
            hideLoadingMessage(tableContainer);
            createAttendanceTable(currentMembers, sundays, currentAttendanceData);
    ...
    function calculateSundays(year, month) { // M/D 형식 반환 (변경 없음)
        const sundays = []; const date = new Date(year, month - 1, 1);
